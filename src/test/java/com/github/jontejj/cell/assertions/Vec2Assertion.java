...
public class Vec2Assertion extends GenericComparableAssert<Vec2>
{
	...
	public Vec2Assertion isEqualTo(Vec2 expected, Double acceptableDelta)
	{
		if(isEqual(expected, acceptableDelta))
			return this;
		// super.failIfCustomMessageIsSet();
		throw failure(unexpectedNotEqual(actual, expected) + format(" using delta:<%s>", acceptableDelta));
	}
	...
	private boolean isEqualTo(double actualDouble, double expected, double deltaValue)
	{
		if(Double.valueOf(actualDouble).compareTo(expected) == 0)
			return true;
		if(Math.abs(expected - actualDouble) <= deltaValue)
			return true;
		return false;
	}

	private static String unexpectedNotEqual(Object actual, Object expected)
	{
